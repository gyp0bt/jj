...
- 仕様書の方針を明確化（properties抽出、スナップショット比較、.jj/config初期化）
- プロジェクト全体の参照を更新
...
**明確化した方針:**
- `.jj/config/` フォルダが存在しない場合のみ初期化を実行
...
- [ ] .jj/configの初期化タイミングとして、他に必要なタイミングはあるか？
...
- .jj/config初期化: フォルダが存在する場合はスキップすると明記
