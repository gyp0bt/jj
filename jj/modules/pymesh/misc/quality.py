...
def get_element_quality(
    ...
    if mode is None:
        mode = ["skewness", "aspect", "detJ", "volume", "area"]
...
def get_triangle_area_batch(p1: np.ndarray, p2: np.ndarray, p3: np.ndarray) -> np.ndarray:
    ...
    Args:
        p1, p2, p3 (np.ndarray): 各三角形の3頂点座標 (n, 3)
...
def get_quadrilateral_area_batch(p1: np.ndarray, p2: np.ndarray, p3: np.ndarray, p4: np.ndarray) -> np.ndarray:
    ...
    Args:
        p1, p2, p3, p4 (np.ndarray): 各四角形の4頂点座標 (n, 3)

    Returns:
        np.ndarray: 面積 (n,)
    """
    area1 = get_triangle_area_batch(p1, p2, p3)
    area2 = get_triangle_area_batch(p1, p3, p4)
    return area1 + area2


def get_area_batch(element_node_coord_array: np.ndarray) -> np.ndarray:
    """
    三角形または四角形要素の面積を一括計算する関数。

    Args:
        element_node_coord_array (np.ndarray): (n, m, 3) 頂点配列（mは頂点数）

    Returns:
        np.ndarray: 各要素の面積 (n,)
    """
    num_nodes = element_node_coord_array.shape[1]

    if num_nodes == 3:
        return get_triangle_area_batch(
            element_node_coord_array[:, 0],
            element_node_coord_array[:, 1],
            element_node_coord_array[:, 2],
        )
    elif num_nodes == 4:
        return get_quadrilateral_area_batch(
            element_node_coord_array[:, 0],
            element_node_coord_array[:, 1],
            element_node_coord_array[:, 2],
            element_node_coord_array[:, 3],
        ...
        raise ValueError(
            f"サポートされていない要素形式です（頂点数: {num_nodes}）。"
            "三角形(3) または 四角形(4) のみに対応しています。"
        )
