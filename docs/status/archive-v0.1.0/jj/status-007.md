...
## 概要
- `.j2/storage` のYAML/JSON保存を担う `GraphStorage` を追加。
...
- `services/storage` に `GraphStorage` を実装し、`.j2/storage/graph.yaml`/`graph.json` の読み書きを追加。
- `docs/detail.md` に `.j2/storage` の保存フォーマット例を追加。
...
- `.j2/storage/graph.yaml` を起点に、`GraphStorage` と `parse` の接続方法を検討してください。
