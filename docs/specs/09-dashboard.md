...
jj側は**即時的な一覧性**（ローカルStreamlit）を、mat-db側は**詳細で高機能なレンダリング**（Next.js）を担う。
...
| mat-db (Next.js) | 高機能ビュー一式、ユーザー管理済み | jjとは別プロジェクト | **将来的な統合先** |

### 役割分担

```
┌─────────────────────────────┐    ┌──────────────────────────────┐
│  jj dashboard (Streamlit)   │    │  mat-db (Next.js)            │
│                             │    │                              │
│  ・プロジェクトローカル      │    │  ・組織横断のDB              │
...
- **認証**: 初期は不要（ローカル専用）、mat-db統合時にAPIキーまたはOAuth
...
mat-db (Next.js)
  ...
  │
  └── API連携
      ├── jj serve → mat-db fetch  # リアルタイム取得
      └── jj export → mat-db upload # バッチアップロード
```

### 4.2 データ変換: jj GraphModel → mat-db GraphData
...
mat-dbの既存GraphData形式へのマッピングルール:
- `JJNode` → mat-dbの`Node`（typeマッピングが必要）
- `JJRelation` → mat-dbの`Edge`（labelをedge_typeに変換）
- `properties` → mat-dbのノードメタデータ

### 4.3 統合方式の選択肢

| 方式 | 説明 | 適用場面 |
|------|------|----------|
| **API連携** | `jj serve` → mat-db がfetch | リアルタイム連携、開発時 |
| **バッチアップロード** | `jj export --target mat-db` → mat-db API | 定期同期、本番運用 |
| **共有DB** | jjがmat-dbのDBに直接書き込み | 将来的な完全統合時 |
...
jj export --target mat-db           # mat-dbアップロード形式
```
...
- [ ] `jj export --target mat-db` の実装
- [ ] mat-db側にjjプロジェクトインポート機能追加
- [ ] API連携（jj serve → mat-db fetch）
- [ ] mat-db既存ビュー（テーブル/カード/グラフ）でjjデータを表示
...
| 出力層 | ← ダッシュボード層 | dashboard-json/mat-dbエクスポートを追加 |
