...
class DecorateMethods(type):
    def __new__(cls, name, bases, dct):
        ...
        return super(DecorateMethods, cls).__new__(cls, name, bases, dct)
...
class OdbWrapper:
    ...
    def get_step(self, step=1):
        ...
        if step_name in self.odb.steps.keys():
            return self.odb.steps[step_name]
        elif step_name in self.get_or_create_scratch_odb().steps.keys():
            return self.get_or_create_scratch_odb().steps[step_name]

    def iter_steps(self):
        for step_name, step in self.odb.steps.items():
    ...
    def get_step_name(step=1, odb_filepath=None):
        ...
        if isinstance(step, int):
            if step < 0:
                ...
                step_name = [i for i in odb.odb.steps.keys()][step]
            else:
    ...
    def get_or_create_session_step(self):
        scratch_odb = self.get_or_create_scratch_odb()
        if C.SESSIONSTEP.NAME not in scratch_odb.steps.keys():
    ...
    def create_field_output(
        self, field, field_name, step=C.SESSIONSTEP.NAME, frame_index=0
    ):
    ...
    def dump_field_to_json(self, field_name, step, frame_index=-1, type="element"):
        ...
        field = self.get_field_output(
            field_name=field_name, step=step, frame_index=frame_index
        )
        step_name = self.get_step_name(step, self.odb_filepath)

        if step_name not in field_dict.keys():
            field_dict[step_name] = dict()

        if frame_index not in field_dict[step_name].keys():
            field_dict[step_name][frame_index] = dict()

        if type == "element":
            field_dict[step_name][frame_index][field_name] = {
                i.elementLabel: i.data for i in field.values
            }
        elif type == "node":
            field_dict[step_name][frame_index][field_name] = {
                i.nodeLabel: i.data for i in field.values
            }

        if isinstance(field.values[0].data, np.ndarray):
            field_dict[step_name][frame_index][field_name] = {
                k: [str(i) for i in v]
                for k, v in field_dict[step_name][frame_index][field_name].items()
            }
    ...
    def get_history(self, refnode, key, step, node_number=0):
        ...
        tipHistories = self.odb.steps[
            self.get_step_name(step, self.odb_filepath)
        ].getHistoryRegion(point=histPoint)
...
def average_by_id(data):
    ...
    unique_ids, start_indices, counts = np.unique(
        ids_sorted, return_index=True, return_counts=True
    )

    result = np.empty((len(unique_ids), values.shape[1] + 1))

    for i, (uid, start, count) in enumerate(zip(unique_ids, start_indices, counts)):
...
def print_time(t1, t2, description):
    ...
    if t2 - t1 > cutoff:
        ...
        if silent is None or not silent:
            print("Processed in {:.1f} sec.".format(t2-t1) + " --- '{}'".format(description))
...
class Step:
    ...
    def to_dict(self):
        ...
        for frame_name_i, frame_i in self.frames.items():
            ...
            t2 = time.time()
            print_time(t1, t2, "Converted frame data to dict ({})".format(frame_name_i))
...
class OdbData:
    ...
    def to_dict(self):
        ...
        for step_name_i, step_i in self.steps.items():
            ...
            t2 = time.time()
            print_time(t1, t2, "Converted step data to dict ({})".format(step_name_i))
        return data
...
class OdbSeriarizer:
    ...
    def __init__(self, odb_filepath):
        ...
        self.data = OdbData()
        for step_name_i, step_i in self.odb.iter_steps():
            ...
            for frame_index in frame_key_list:
                ...
                for field_key in field_key_list:
                    ...
                    if not globals()["silent"]:
                        print_msg(
                            "Getting value of "
                            + str((step_name, frame_index, field_key))
                        )

                    field = frame.fieldOutputs[field_key]
                    self.data.steps[step_name].frames[frame_index].fields[
                        field_key.replace(" ", "")
                    ].field = field
                    ...
                    for i in ["S", "LE"]:
                        ...
                        for k in keys:
                            ...
                            self.data.steps[step_name].frames[frame_index].fields[
                                k
                            ].field = frame.fieldOutputs[i].getScalarField(mode)

                        t2 = time.time()
                        print_time(t1, t2, "Got field data '{}'".format(keys))
    ...
    def get_histories(
        ...
        for step_name, step in self.odb.iter_steps():
            if step_name_list is not None and step_name not in step_name_list:
                continue
            for refnode_key, endSet in self.odb.odb.rootAssembly.instances[
                "PART-1-1"
            ].nodeSets.items():
                ...
                for refnode_number, refnode in enumerate(endSet.nodes):
                    if (
                        refnode_number_list is not None
                        and refnode_number not in refnode_number_list
                    ):
                        continue
                ...
                for history_key, history_data in tipHistories.historyOutputs.items():
                    if (
                        history_key_list is not None
                        and history_key not in history_key_list
                    ):
                        continue
                    data = [[i[0], i[1]] for i in history_data.data]
                    self.data.steps[step_name].histories[refnode_key].histories[
                        int(refnode_number)
                    ].histories[history_key] = data
...
if __name__ == "__main__":
    ...
    args.refnode_numbers = (
        [int(i) for i in args.refnode_numbers]
        if args.refnode_numbers is not None
        else None
    )
