    ...
    def render_page(
        ...
        **kwargs: Any,
        ...
        with col2:
            ...
            x_key = st.selectbox("X軸", prefix_keys, index=1)
        with col3:
            ...
            if not y_options:
                ...
                return
            # TODO: configでデフォルトキー指定できるようにする
            index = min(len(y_options) - 1, 0)
            y_keys = st.multiselect("Y軸", y_options, default=y_options[index])
        ...
        with st.expander("軸範囲設定", expanded=False):
            ...
            with rc4:
                ax_y_max = st.number_input("Y最大", value=100, key="_ap_y_max", format="%g")
